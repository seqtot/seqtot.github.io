...
export class WebAudioFontLoader {
	...
	instrumentTitles = function (): string[] {
		if (this.instrumentNamesArray.length == 0) {
			this.instrumentNamesArray = getInstrumentTitles();
		}
	...
	getInstrumentsByGroup(group: number | string): { variable: string, url: string, title: string, pitch: number, index: number}[] {
		const result = [];
		const groupInd = parseInteger(group, null);

		if (groupInd === null) {
			return result;
		}

		this.instrumentKeys().forEach((key, index) => {
			const currGroupInd = parseInteger(key.substring(0, 3));

			if (groupInd === currGroupInd) {
				result.push({
					variable: '_tone_' + key,
					url: 'https://surikov.github.io/webaudiofontdata/sound/' + key + '.js',
					title: this.instrumentTitles()[groupInd],
					index,
					pitch: -1
				})
			}
