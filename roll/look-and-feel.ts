...
export class LookAndFeel_Default implements LookAndFeel {
  ...
  public drawNote(
    ...
    if (height < 4 && !selected) {
      ...
      g.strokeStyle = selected
        ? colors.noteOutlineSelected
        : colors.noteOutline;
      g.stroke();
    }
  }

  public drawVelocityHandle(
    g: CanvasRenderingContext2D,
    x: number,
    note: Note,
    width: number,
    ...
    const w = Math.max(
      0,
      hScale * (note.tempDuration ? note.tempDuration : note.duration)
    );
    const y = height - note.velocity * vScale;
    const endingH = 6;

    const color = note.selected
      ? colors.velocityHandleSelected
      : colors.velocityHandle;
  ...
  public drawPianoRoll(
    ...
    for (let i = Math.floor(start); i <= Math.ceil(end); ++i) {
      ...
      g.fillRect(width / 2, y - semiHeight, width / 2, semiHeight);

      // stroke
      g.fillStyle = colors.strokeLight;
      g.fillRect(width / 2, y - semiHeight, width / 2, 1);
    }
  }

  public isOnPianoRoll(
    x: number,
    y: number,
    width: number,
    height: number,
