...
import {IPoint, Rector} from './types';
...
export class RootComponentHolder<T extends CanvasComponent> {
  ...
  public resize(width: number, height: number): void {
    ...
    this.rootComponent.setBounds(new Rector(0, 0, width, height));
  }

  public initMouseEventListeners(): void {
    let pressedComponent: CanvasComponent = null;
    let componentUnderMouse: CanvasComponent = null;
    let mouseDownPos: IPoint;
    let mouseUpPos: IPoint;
    let mouseDownTime: number;
    let mouseUpTime: number;
    let consecutiveClickCount: number = 0;
    let consecutivePressCount: number = 0;
    let lastClickTime: number;
    let lastClickPos: IPoint;
    ...
    const hit = (event: MouseEvent, action: (hit: CanvasComponent) => void) => {
    ...
    this.documentMouseUpListener = (mouseEvent: MouseEvent) => {
      ...
      if (pressedComponent != null) {
        ...
        if (mouseUpTime < mouseDownTime + CLICK_INTERVAL && !wasDragged) {
          ...
          if (
            lastClickTime == null ||
            mouseUpTime > lastClickTime + DOUBLE_CLICK_INTERVAL
          ) {
          ...
          if (consecutiveClickCount == 2 && !wasDragged) {
    ...
    this.documentMouseMoveListener = (mouseEvent: MouseEvent) => {
      ...
      if (
        !wasDragged &&
        mouseDownPos != null &&
        squaredDistance(mouseDownPos.x, mouseDownPos.y, x, y) >
          CLICK_MAX_DISTANCE_SQUARED
      ) {
        wasDragged = true;
      }
...
function extractModifiers(mouseEvent: MouseEvent): {
  shift: boolean;
  option: boolean;
} {
  return {
